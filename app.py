...
# FIX FOR IMAGEMAGICK ON LINUX (STREAMLIT CLOUD)
# We need to tell the system where the 'convert' binary is.
if os.name == 'posix': # If running on Linux/Mac
...
def generate_video(text_input, font_path):
    ...
    for idx, chunk_index in enumerate(range(0, len(words), WORDS_PER_SCREEN)):
        ...
        # Background
        bg_clip = mp.ColorClip(size=VIDEO_SIZE, color=(0,0,0)).set_duration(batch_duration)
        screen_clips.append(bg_clip)
        
        # Vertical Centering
        total_block_height = (LINES_PER_SCREEN * (FONT_SIZE * 1.2)) + ((LINES_PER_SCREEN - 1) * LINE_GAP)
        ...
        for line_idx in range(0, len(batch_words), WORDS_PER_LINE):
            ...
            for w in line_words:
                try:
                    # Try using the custom font
                    temp_clip = mp.TextClip(w, fontsize=FONT_SIZE, font=font_path, color='white')
                except Exception:
                    # Fallback if font fails
                    temp_clip = mp.TextClip(w, fontsize=FONT_SIZE, font='Liberation-Sans', color='white')
            ...
            for i, (word_text, w_width, w_height) in enumerate(word_clips_data):
                ...
                # Dim Word (25% opacity)
                dim_clip = mp.TextClip(word_text, fontsize=FONT_SIZE, font=font_path, color='white')\
                    .set_position((current_x, pos_y)).set_duration(batch_duration).set_opacity(0.25)
                
                # Bright Word
                bright_duration = batch_duration - light_up_time
                if bright_duration > 0:
                    bright_clip = mp.TextClip(word_text, fontsize=FONT_SIZE, font=font_path, color='white')\
                        .set_position((current_x, pos_y)).set_start(light_up_time).set_duration(bright_duration)
        ...
        screen_composite = mp.CompositeVideoClip(screen_clips, size=VIDEO_SIZE).set_duration(batch_duration)
    ...
    if final_clips:
        ...
        # WRITE WITH AAC FOR COMPATIBILITY
